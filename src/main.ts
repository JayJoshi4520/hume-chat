  ...
  async function connect(): Promise<void> {
    ...
    if (!client) {
      client = new HumeClient({
        apiKey: "V2EHTxPkLdNbrKeCFTD0B5WjQySGkO1TnbHbWsV1fmi7XR6z",
        secretKey: "ac34PzKp2OOuZIVXwRHbjGRa0hoGudI9JK6HaLY9QecJU9AyIyPjvNoN1EZ2dLes",
      });
    }

    // instantiates WebSocket and establishes an authenticated connection
    socket = await client.empathicVoice.chat.connect({
      configId: '379dd9c3-c766-4dae-ae98-fd5f05e16f92',
  ...
  function handleWebSocketMessageEvent(message: Hume.empathicVoice.SubscribeEvent): void {
    ...
    switch (message.type) {
      ...
      case 'assistant_message':
        ...
        let text = `{"role": "${role}", "message": "${content}" }`
        chatLog.push(JSON.parse(text))
        console.log(text)
